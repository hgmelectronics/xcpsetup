...
class XCPConnection(object):
    ...
    def byteToAG(self, bytes):
        return int(bytes / self._addressGranularity)
    ...
    def _query(self, action_func, *args):
        ...
        while failures <= maxFailures:
            ...
            except (XCPTimeout, XCPPacketLost):
                failures += 1
    ...
    def _action_upload8(self, ptr):
        if self._addressGranularity == 1:
            ...
            if reply[0] != 0xFF:
                ...
                self._calcMTA = XCPPointer(ptr.addr + 1, ptr.ext)
                return reply[1]
        else:
            raise XCPInvalidOp()
    ...
    def _action_upload16(self, ptr):
        ...
        if reply[0] != 0xFF:
            ...
            self._calcMTA = XCPPointer(ptr.addr + self.byteToAG(2), ptr.ext)
            return reply[1]
    ...
    def _action_upload32(self, ptr):
        ...
        if reply[0] != 0xFF:
            ...
            self._calcMTA = XCPPointer(ptr.addr + self.byteToAG(4), ptr.ext)
            return reply[1]
    ...
    def _action_program_block(self, ptr, data):
        ...
        while remBytes > 0:
            ...
            if len(replies) > 0:
                ...
                for reply in replies:
                    if len(reply) < 2 or reply[0] != 0xFE or reply[1] != 0x29:
                        raise XCPBadReply() # not ERR_SEQUENCE
            ...
            self._calcMTA = XCPPointer(self._calcMTA.addr + self.byteToAG(payloadBytes), self._calcMTA.ext)
    ...
    def _action_program_packet(self, ptr, data):
        ...
        if reply[0] != 0xFF:
            ...
            self._calcMTA = XCPPointer(ptr.addr + self.byteToAG(len(data)), ptr.ext)
    ...
    def program_range(self, ptr, data):
        ...
        if self._pgmMasterBlockMode:
            while remBytes > 0:
                ...
                blockStartPtr = XCPPointer(ptr.addr + self.byteToAG(blockStartBytes), ptr.ext)
            ...
            while remBytes > 0:
                ...
                packetStartPtr = XCPPointer(ptr.addr + self.byteToAG(packetStartBytes), ptr.ext)
