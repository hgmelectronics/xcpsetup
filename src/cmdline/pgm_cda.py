...
parser = argparse.ArgumentParser(description="programs application code into a CDA")
...
parser.add_argument('-i', help="Target {0|1|recovery}", dest="targetID", default=None)
...
with CANInterface.MakeInterface(args.deviceType, args.deviceName) as interface:
    if targetIDs == None:
        ...
        for targetID in targetIDs:
            if targetID >= 0:
                targetSlaves.append((CANInterface.XCPSlaveCANAddr(0x9F000080 + 2 * targetID, 0x9F000081 + 2 * targetID), targetID))
            else:
        ...
        while 1:
            try:
                ...
                if args.crcMatch:
                    ...
                    conn.program_start()
                    if conn.program_check(XCPConnection.Pointer(0x08005000, 0), len(data), dataCRC):
                        print('CRC matched flash contents')
                        conn.program_reset()
                        break
                conn.program_app(XCPConnection.Pointer(0x08005000, 0), data, dataCRC)
                print('Program OK')
                break
